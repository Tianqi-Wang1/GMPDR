...
def train(model, train_loader, train_images, e, obs_len, pred_len, batch_size, params, gt_template, device,
		  input_template, optimizer, criterion, dataset_name, homo_mat, task_id):
	train_loss = 0
	train_ADE = []
	train_FDE = []
	model.train()
...
def get_feature(model, train_loader, train_images, obs_len, pred_len, batch_size, params, gt_template, device,
				input_template, optimizer, dataset_name, homo_mat, task_id):

	features_all = torch.tensor([]).to(device)
	with torch.no_grad():
		counter = 0
		# outer loop, for loop over each scene as scenes have different image size and to calculate segmentation only once
		for batch, (trajectory, meta, scene) in enumerate(train_loader):
			...
			for i in range(0, len(trajectory), batch_size):
				...
				features = features[5].reshape(batch_size, -1)
				pad_size = 12288 - features.size(1)

				features_padded = F.pad(features, (0, pad_size), mode='constant', value=0)

				features_all = torch.cat((features_all, features_padded))
	return features_all
